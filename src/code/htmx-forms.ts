export const baseHtml = `function BaseHtml({ children }: html.PropsWithChildren) {
  return \`<!DOCTYPE html>
  <html lang="en">
  <head>
    <meta charset="UTF-8">
...
export const renderForm = `app.get('/', (_req, res) => {
  res.send(
    <BaseHtml>
      <body>
...
export const registrationForm = `import { clsx } from 'clsx';
...
export const registerEndpoint = `import { z } from 'zod';
...
export const happyPath = `app.post('/register', (req, res) => {
...
export const formErrors = `app.post('/register', (req, res) => {
  const result = RegisterBodySchema.safeParse(req.body);
  if (result.success) {
    // the happy path
  } else {
...
export const renderingErrors = `<div>
  <h2 class="card-title">Register</h2>
  <form hx-post="/register" hx-target="closest div" hx-swap="outerHTML">
    <div class="form-control w-full max-w-xs">
