...
Thinking in HTMX requires a paradigm shift from frameworks like React, Vue and it's fun when you can directly return HTML back to the browser (**the OG SSR way**) by not sacrificing the benefits of JSX! HTMX directives mixed with a pinch of custom JS via Hyperscript makes web dev fun again!

I hope you liked the post, follow me on LinkedIn and Twitter for where I will continue posting about this series. The next part will include a persistent database like DynamoDB so that the todos aren't lost on server restart. Happy HTMXing 😃
