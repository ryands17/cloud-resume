...
It's truly a joy working with Rust which isn't just for low level software but also for full fledged APIs and web apps. There's a lot more we can do with Axum like serve static files, server render applications, and more. I will also be creating another blog post on how we can create server rendered apps with Axum.
